...
PYBIND11_MODULE(cpp_underwriter, m) {
  ...
  py::class_<ExperienceRating>(m, "ExperienceRating")
      .def(py::init<double, double>(), py::arg("k_parameter") = 1082.0, py::arg("ballast") = 0.0)
      .def("credibility_weighted_rate", &ExperienceRating::credibility_weighted_rate, py::arg("observed_rate"), py::arg("expected_rate"), py::arg("credibility_z"))
      .def("calculate_credibility", &ExperienceRating::calculate_credibility, py::arg("claim_count"))
      .def("experience_mod_factor", &ExperienceRating::experience_mod_factor, py::arg("actual_losses"), py::arg("expected_losses"))
      .def("buhlmann_straub_credibility", &ExperienceRating::buhlmann_straub_credibility, py::arg("exposure"), py::arg("variance_ratio"))
      .def("expected_losses", &ExperienceRating::expected_losses, py::arg("exposure"), py::arg("frequency"), py::arg("severity"));

  py::class_<ExposureRating>(m, "ExposureRating")
      .def(py::init<double, double>(), py::arg("base_limit") = 100000.0, py::arg("b_parameter") = 1.0)
      .def("increased_limits_factor", &ExposureRating::increased_limits_factor, py::arg("target_limit"))
      .def("layer_premium", &ExposureRating::layer_premium, py::arg("ground_up_premium"), py::arg("attachment"), py::arg("limit"))
      .def("burning_cost", &ExposureRating::burning_cost, py::arg("historical_losses"), py::arg("attachment"), py::arg("limit"), py::arg("exposure_years"))
      .def("loss_elimination_ratio", &ExposureRating::loss_elimination_ratio, py::arg("deductible"), py::arg("expected_loss"), py::arg("severity_cv"))
      .def("excess_risk_load", &ExposureRating::excess_risk_load, py::arg("layer_std_dev"), py::arg("risk_load_multiple"));

  py::class_<RateAnalyzer>(m, "RateAnalyzer")
      .def(py::init<double, double>(), py::arg("target_combined_ratio") = 1.0, py::arg("expense_ratio") = 0.3)
      .def("on_level_factor", &RateAnalyzer::on_level_factor, py::arg("rate_changes"))
      .def("rate_change_impact", &RateAnalyzer::rate_change_impact, py::arg("old_premium"), py::arg("new_premium"))
      .def("combined_ratio", &RateAnalyzer::combined_ratio, py::arg("loss_ratio"))
      .def("required_rate_change", &RateAnalyzer::required_rate_change, py::arg("current_loss_ratio"))
      .def("on_level_premiums", &RateAnalyzer::on_level_premiums, py::arg("earned_premiums"), py::arg("rate_changes"))
